...
export const productUpdate = async(req, res) => {
    try {
        ...
        let updateProduct = await Product.findOneAndUpdate(
            ...
            data,
            { new: true}
            // obtener la información de la categoría relacionada con el producto actualizado
            ).populate('category', ['nameCategory', '-_id']).select('-__v')
...
export const searchProduct = async (req, res) =>{
    try {
        ...
        if(product.length == 0) return res.status(404).send({message: 'Products not found'})
